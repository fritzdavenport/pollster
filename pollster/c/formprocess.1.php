				...
				if ($q === false) {
					...
					if ($q === false) { //table didn't exist
						...
						if ($DEBUG && $qta){
							...
							if ($DEBUG){
								echo "Something went wrong with table creation";
							}
						}
						if ($DEBUG){ 
							$db->queryExec('INSERT into Question (QuestionText) VALUES ("This is an example")'); 
						}
					...
					if ($DEBUG){echo "table Question was made previously.</br>";
					} 
				} // end if table wasn't made or qn non-existant
